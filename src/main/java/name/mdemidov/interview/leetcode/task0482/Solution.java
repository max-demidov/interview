package name.mdemidov.interview.leetcode.task0482;
...
public class Solution {
    ...
    public static void main(String[] args) {
        System.out.println(new Solution().licenseKeyFormatting("5F3Z-2e-9-w", 4)); // "5F3Z-2E9W"
        System.out.println(new Solution().licenseKeyFormatting("2-5g-3-J", 2)); // "2-5G-3J"
    }

    public String licenseKeyFormatting(String S, int K) {
        String s = S.replace("-", "").toUpperCase();
        int firstGroup = s.length() % K;
        StringBuffer sb = new StringBuffer(firstGroup > 0 ? s.substring(0, firstGroup) : "");
